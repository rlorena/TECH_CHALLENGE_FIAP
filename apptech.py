...
#Gráfico de barras demonstrando a quantidade e o valor exportado do período
        ...
        with col2:
                ...
                fig.update_layout(title='Comparativo entre os 5 maiores importadores.', width = 700)
                fig.update_xaxes(title_text="Ano")
                fig.update_yaxes(title_text="Valor")
                fig.show()
                st.plotly_chart(fig)
                st.write(
                '###### Observamos neste gráfico que o comportamento apresentado nas exportações a Rússia foi \
                        inverso ao restante do grupo analisado, ou seja, a quantidade exportada em litros superou \
                        valor total vendido.'
                        ) 
                st.divider()
